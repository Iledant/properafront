<template>
  <v-dialog :value="value" @input="$emit('input', false)" max-width="600px" persistent>
    <v-card>
      <v-card-title class="primary white--text">Ratios de « {{ ratios.fullName }} »</v-card-title>
      <v-container grid-list-md fluid>
        <v-layout row>
          <v-flex xs6>
            <v-text-field label="Taux 75" v-model="fmtR75" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 77" v-model="fmtR77" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 78" v-model="fmtR78" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 91" v-model="fmtR91" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 92" v-model="fmtR92" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 93" v-model="fmtR93" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 94" v-model="fmtR94" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs6>
            <v-text-field label="Taux 95" v-model="fmtR95" v-percentage-input reverse prefix="%" />
          </v-flex>
          <v-flex xs12>Total : {{ fmtTotal }}</v-flex>
          <v-flex xs12 class="error" v-show="badTotal">Le total doit faire 100 %</v-flex>
        </v-layout>
      </v-container>
...
export default {
  ...
  computed: {
    total () {
      return 0.01 * (this.parse(this.fmtR75) +
        this.parse(this.fmtR77) +
        this.parse(this.fmtR78) +
        this.parse(this.fmtR91) +
        this.parse(this.fmtR92) +
        this.parse(this.fmtR93) +
        this.parse(this.fmtR94) +
        this.parse(this.fmtR95))
    },
    badTotal () {
